...
export const VkApiInterceptor = {
  request: (logger: LokiLogger) => {
    return {
      ...
      onRejected: (error) => {
        logger.error('REQUEST TO VK API', { errorData: error });
  ...
  response: (logger: LokiLogger) => {
    return {
      onFulfilled: (resp) => {
        ...
        logger.logResponse(baseURL + url, method, status, { data, statusText, headers }, 'RESPONSE FROM VK API');
      ...
      onRejected: (error) => {
        ...
        const errorMsg = `RESPONSE FROM VK API [${config?.method?.toUpperCase()}] ${config.baseURL + config.url} - ${status}: ${statusText} (${
